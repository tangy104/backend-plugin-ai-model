...
# def process_audio(audio_path, audio_path_wav):
def process_audio(audio_path, audio_path_wav):
    ...
    # System message for grammatical issues
    system_message = """
    You are an expert at speaking and understanding english.
    For the given text, return an answer pointing out all grammatical issues in the text.
    ...
    response = model.invoke({"input": input_text})

    # Calculate error count and error density
    prompt2 = ChatPromptTemplate.from_messages([("system", "You are an expert at extracting numbers from a given text. For a given text, just output the number of errors in it. The output should be a single integer only."), ("human", "{response}")])
    chain2 = prompt2 | llm | StrOutputParser()
    errors = int(chain2.invoke({"response": response}))
    total_words = len(input_text.split())

    # Process word clarity and pronunciation accuracy
    confidence_thresholds = {"Perfect": 85, "Very Good": 70, "Good": 60, "Fair": 50}
    ...
    gunning_index = compute_gunning_index(input_text)
    error_density, fig3 = compute_error_density(errors, total_words)
    wps, fig4 = compute_words_per_second(audio_path, input_text)
    ...
    # else:
    #     json_data["smr"] = ["No SMR Data available", 0]

    json_data = {
    "mfccs": [float(mfccs.mean()), b1],
    "zcr": [float(zcr.mean()), b2],
    "gunning_index": [float(gunning_index)],
    "error_density": [float(error_density), b3],
    "wps": [float(wps), b4],
    "avg_pause": [float(avg_pause), b5],
    "punc_acc": [b6]
}

    if res:
        json_data["smr"] = [res, float(smr)]
    else:
        json_data["smr"] = ["No SMR Data available", 0]
    ...
    """
    prompt1 = ChatPromptTemplate.from_messages([("system", system1)])
    chain1 = prompt1 | llm

    # Parallel feedback chains for pros, cons, and summary
    systempros = "Extract pros of the {feedback}, output only the list of the pros, ""NOTHING ELSE"""
    systemcons = "Extract cons of the {feedback}, output only the list of the cons, ""NOTHING ELSE"""
    systemsummary = "Generate a 50 words summary for the {feedback}."
    ...
    # Create parallel runnable to execute chains in parallel
    ...
    # return b, json_data, json_data2
    return json_data, json_data2

# Example usage
# audio_path = "path_to_audio_file"
# b, json_data, json_data2 = process_audio(audio_path)
